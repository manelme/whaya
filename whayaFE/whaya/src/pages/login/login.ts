import { Component } from '@angular/core';
import { IonicPage, NavController, NavParams, Platform, ViewController } from 'ionic-angular';
import { Camera, CameraOptions } from '@ionic-native/camera';
import { Storage } from '@ionic/storage';
import { AlertController } from 'ionic-angular';

import { AuthServiceProvider } from '../../providers/auth-service/auth-service';
import { NeoServiceProvider } from '../../providers/neo-service/neo-service';
import { TabsPage } from '../tabs/tabs';
import { RegisterPage } from '../register/register';
import { GeolocProvider } from '../../providers/geoloc/geoloc';

/**
 * Generated class for the LoginPage page.
 *
 * See https://ionicframework.com/docs/components/#navigation for more info on
 * Ionic pages and navigation.
 */

@IonicPage()
@Component({
  selector: 'page-login',
  templateUrl: 'login.html',
})
export class LoginPage {

  email: string;
  name: string;
  password: string;
  error: string = "";


  constructor(public navCtrl: NavController, public navParams: NavParams, public storage: Storage, private geolocProvider: GeolocProvider, public alertCtrl: AlertController, private authService: AuthServiceProvider) {

  }

  ionViewDidLoad() {
    console.log('ionViewDidLoad LoginPage');
    this.geolocProvider.getUserPosition().then(response =>{
      console.log(response);
    })
  }


  registerUser() {
    this.navCtrl.push(RegisterPage);

  }

  loginUser() {

    /* let prompt = this.alertCtrl.create({
      title: 'Login',
      message: "Enter the name that you want the other users to know you for:",
      inputs: [
        {
          name: 'name',
          placeholder: 'name'
        },

      ],
      buttons: [
        {
          text: 'Cancel',
          handler: data => {
            console.log('Cancel clicked');
          }
        },
        {
          text: 'Save',
          handler: data => {
            console.log('Saved clicked');
            
        }
      ]
    });

    prompt.present();
    */
    this.authService.loginUser(this.email, this.password).then((data2: Response) => {
      let data:any = data2.json();
      data.email= this.email;
      this.storage.set("currentUserAuth", data);   
      this.navCtrl.push(ModalContentPage);
    }, (err) => {

      this.error = 'Error!!: ' + err;

    });
  }
    
}

@Component({
  selector: 'modal-page',
  template: `
  <ion-header>
  <ion-navbar color="primary">
    <ion-title>
      Settings
    </ion-title>
    <ion-buttons end>
      <button ion-button icon-only *ngIf="name!=''" (click)="save()">
        <ion-icon name="checkmark"></ion-icon>
      </button>
    </ion-buttons>
  </ion-navbar>
</ion-header>

<ion-content style=" width: 100%;height: 100%;background: linear-gradient(10deg,#4E5EFC, #4EE7FC);" padding>
  <ion-card style="background: rgba(232,230,230,0.6);
  border-radius: 10px;">  
  <ion-card-content>
    <ion-item style="background: rgba(232,230,230,0);">
    <ion-label floating >Username </ion-label>
    <ion-input style="border-bottom: #4E5EFC solid 1px;" type="text" [(ngModel)]="name"></ion-input>
    </ion-item >
    <ion-item style="background: rgba(232,230,230,0);" (click)="getPicture()">
    <ion-icon name="camera" item-start></ion-icon>
    Take a picture  
    </ion-item>
    <svg id="graph" width="100%" height="400px">
  
    <!-- pattern -->
    <defs>
      <pattern id="image" x="0%" y="0%" height="100%" width="100%"
               viewBox="0 0 512 512">
        <image x="0%" y="0%" width="512" height="512" attr.xlink:href="{{image}}" xlink:href=""></image>
      </pattern>
    </defs>
      
    <circle id="sd" class="medium" cx="50%" cy="50%" r="50%" fill="url(#image)" stroke="lightblue" stroke-width="0.5%" />
  </svg>
    </ion-card-content>
  </ion-card>
  
  
</ion-content>
`
})
export class ModalContentPage {
  name;
  image = "";
  userAuth;

  constructor(public platform: Platform, public navCtrl: NavController, public params: NavParams, public viewCtrl: ViewController, private camera: Camera, public storage: Storage, private neoService: NeoServiceProvider) {
    this.name = '';
    this.image = 'data:image/png;base64,';
  }

  getPicture() {
    let options: CameraOptions = {
      destinationType: this.camera.DestinationType.DATA_URL,
      targetWidth: 1000,
      targetHeight: 1000,
      cameraDirection: 1,
      quality: 100
    }
    this.camera.getPicture(options)
      .then(imageData => {
        this.image = `data:image/jpeg;base64,${imageData}`;
      })
      .catch(error => {
        console.error(error);
      });
  }
  ionViewDidEnter(){

    this.storage.ready().then(() => {
      this.storage.get("currentUserAuth").then((data : any) => {
        this.userAuth = data;
      })
    });
  }
  save(){
    this.neoService.addUser(this.userAuth.email, this.name, this.image).then((data3: Response) => {
      
      
              this.storage.set("currentUser", data3.json());
              console.log("Added OK")
              this.navCtrl.setRoot(TabsPage);
              
      
            }, (err) => {

              this.neoService.getUserByEmail(this.userAuth.email).then((data4: Response) => {
                
                let aux: any = data4.json();
                console.log(aux)
                this.neoService.updateUser(aux.id, this.name, this.image).then((data5: Response)=>{
                  this.storage.set("currentUser", data5.json()).then(()=>{
                    console.log("Loadded OK")
                    this.navCtrl.setRoot(TabsPage);
                    window.location.reload()
                  });
                  
                  
                }),(err)=>{
                  this.storage.set("currentUser", data4.json()).then(()=>{
                    console.log("Loadded OK")
                  this.navCtrl.setRoot(TabsPage);
                  window.location.reload()
                  });
                  
                }

                
              }, (err) => {
                console.log(err);
              })
      });
  }
  dismiss() {
    this.viewCtrl.dismiss();
  }
}